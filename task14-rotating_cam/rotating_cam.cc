...
struct Settings {
  std::string const title         = "INF515-ModelViewProjection";
...
// ------------------------------------------------------------------------ //
void
rotateCamera(Shader const& shader, Settings const& setup) {
  // --- ToDo: implement rotation of camera around cube center
  // --- ToDo: use glm to define transformation
  // --- ToDo: use glfwGetTime() to rotate cube
  // --- ToDo: use shader.setMat4fv to send transformation to vertex shader
}
...
renderLoop(Settings& setup) {
  ...
  while (!glfwWindowShouldClose(setup.window)) {
    ...
    // --- clear the window --- //
    // --- ToDo: clear color buffer and depth buffer
    shader.use();
    glBindVertexArray(VAO);
    rotateCamera(shader, setup);
