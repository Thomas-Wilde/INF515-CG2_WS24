...
struct Settings {
  std::string const title         = "INF515-RotatingCam";
    ...
    for (int y = 0; y < height; ++y)
      for (int z = 0; z < 4; ++z) {
        ...
        if (x < 5 || x > (width - 5) || y < 5 || y > (height - 5))
          screen2[z + x * 4 + (height - y - 1) * width * 4] = z == 3 ? 255 : 0;
      }
  // --- generate filename
...
renderLoop(Settings& setup) {
  ...
  while (!glfwWindowShouldClose(setup.window)) {
    ...
    // ---
    glfwGetWindowSize(setup.window, &setup.window_width, &setup.window_height);
    shader.setInt("window_width", setup.window_width);
    shader.setInt("window_height", setup.window_height);
